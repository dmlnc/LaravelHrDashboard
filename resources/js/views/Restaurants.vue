...
<template>
    <div>
    	<RestaurantForm :visible="formVisible" :id="formId" @close="hideForm" @save="loadData"/>

        <a-typography-title :level="5" class="mb-40">Рестораны</a-typography-title>
        ...
        <a-row :gutter="24">
            ...
            <a-col :span="24" :lg="12" :xl="8" class="mb-24" v-for="restaurant in restaurants" :key="restaurant.id">
            	<a-skeleton active :loading="loading" >
                <a-card>

                    <template class="ant-card-actions" #actions>
                    	<div @click="showForm(restaurant.id)"><svg viewBox="64 64 896 896" data-icon="edit" width="1em" height="1em" fill="currentColor" aria-hidden="true" focusable="false" class=""><path d="M257.7 752c2 0 4-.2 6-.5L431.9 722c2-.4 3.9-1.3 5.3-2.8l423.9-423.9a9.96 9.96 0 0 0 0-14.1L694.9 114.9c-1.9-1.9-4.4-2.9-7.1-2.9s-5.2 1-7.1 2.9L256.8 538.8c-1.5 1.5-2.4 3.3-2.8 5.3l-29.5 168.2a33.5 33.5 0 0 0 9.4 29.8c6.6 6.4 14.9 9.9 23.8 9.9zm67.4-174.4L687.8 215l73.3 73.3-362.7 362.6-88.9 15.7 15.6-89zM880 836H144c-17.7 0-32 14.3-32 32v36c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-36c0-17.7-14.3-32-32-32z"></path></svg></div>
                    	<!-- <router-link class="text-primary"  :to="{ name: 'Faculties-page', params: {restaurant_id : restaurant.id}}"> Факультеты </router-link> -->
                        <a-popconfirm
                          :title="'Уверены, что хотите удалить '+restaurant.name+ '?'"
                          ok-text="Да"
                          cancel-text="Нет"
                          @confirm="deleteInstance(restaurant.id)"
                        >
                    ...
                    <a-card-meta :title="'Ресторан '+ restaurant.name" description="">
                    </a-card-meta>
                    <p></p>
                    <a-tag v-for="vacancy in restaurant.vacancies" color="cyan">{{vacancy.name}}</a-tag>
                </a-card>
            	</a-skeleton>
            </a-col>
            <a-col :span="24" :lg="12" :xl="8" class="mb-24">
                ...
                <a-card>
                    ...
                    <a-card-meta title="Создать ресторан" description="">
                    </a-card-meta>
                </a-card>
            	</a-skeleton>
            </a-col>
    ...
    methods: {
    	...
    	loadData(){
            ...
            .then(response => {
                ...
                // router.push({ name: 'restaurant', params: {restaurant_id: } })
            })
            .catch(error => {
                notification.error({
                    message: 'Ошибка',
        ...
        deleteInstance(id){
            ...
            this.$axios.delete('/restaurant/'+id)
            .then(response => {
